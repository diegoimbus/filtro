...
public class IncommController {
    ...
    public IncommController(@NotNull IncommService pincommService,
                            @NotNull IncommProperties incommProperties) {
        super();
        this.incommService = pincommService;
        this.incommProperties = incommProperties;
    }
    ...
    public final Mono<Response> processPinesActivateTransaction(@NotNull @RequestBody Mono<Request> request) {
        return incommService.processPinesActivate(request);
    }

    @DeleteMapping(value = "${spring.application.services.rest.pines.inactivate}")
    public final Mono<Response> processPinesInactivateTransaction(@NotNull @RequestBody Mono<Request> request) {
        return incommService.processPinesReverse(request);
    }

    @PostMapping(value = "${spring.application.services.rest.pines.process}")
    public final Mono<Response> processPin(@NotNull @RequestBody Mono<Request> pRequest) {
    ...
    public final Mono<Response> processActivateCard(@NotNull @RequestBody Mono<Request> request) {
        return incommService.processCardActivate(request);
    }

    @DeleteMapping(value = "${spring.application.services.rest.card.inactivation}")
    public final Mono<Response> processInactivationCard(@NotNull @RequestBody Mono<Request> request) {
        return incommService.processCardInactivate(request);
    }

    @PostMapping(value = "${spring.application.services.rest.card.process}")
    public final Mono<Response> processCard(@NotNull @RequestBody Mono<Request> pRequest) {
        return pRequest.flatMap(request -> {
            Mono<Response> response;
            try {
                Input data = Input.parseInput(request.getData(), request.getMeta());
    ...
    public final Mono<Response> processReversionCard(@NotNull @RequestBody Mono<Request> request) {
