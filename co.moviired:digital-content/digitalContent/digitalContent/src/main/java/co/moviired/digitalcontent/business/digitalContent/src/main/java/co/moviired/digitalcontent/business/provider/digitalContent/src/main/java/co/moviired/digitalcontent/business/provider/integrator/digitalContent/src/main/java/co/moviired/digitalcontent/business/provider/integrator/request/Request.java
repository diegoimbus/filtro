...
public class Request implements IRequest {
    ...
    public final Request parseRequestCardInactivate(DigitalContentRequest data,
        ...
        parametersData.put("customerTxReference", data.getCorrelationId());
        parametersData.put("phoneNumber", data.getPhoneNumber());
        parametersData.put("productId", data.getProductId());
        parametersData.put("EANCode", data.getEanCode() + "|" + data.getCardSerialNumber() + "#" + data.getOperation());
        ...
        // META
        String ip = (data.getIp() == null) ? "127.0.0.1" : data.getIp();

        parametersMeta.put("systemId", "9006");
        parametersMeta.put("originAddress", ip);

        try {
            parametersMeta.put("requestDate", formatDateDestino.format(dateFormat.parse(data.getIssueDate())));
        } catch (ParseException e) {
            parametersMeta.put("requestDate", formatDateDestino.format(new Date()));
        }
        parametersMeta.put("customerId", "174484");
        try {
            parametersMeta.put("requestReference", formatDateDestino2.format(dateFormat.parse(data.getIssueDate())));
        } catch (ParseException e) {
