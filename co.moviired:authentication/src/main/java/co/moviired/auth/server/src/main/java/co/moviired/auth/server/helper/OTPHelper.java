...
import co.moviired.auth.server.domain.dto.GenerateOtpRequest;
...
import co.moviired.auth.server.providers.supportotp.response.SupportOTPResponse;
...
public final class OTPHelper implements Serializable {
    ...
    private static final String FINALIZE = "finalized";
    ...
    public Mono<SupportOTPResponse> generateOTP(@NotNull String source, @NotNull User user) {
        // Establecer el template del SMS
        String template = supportSmsProperties.getOtpMoviiredTemplate();
        Boolean sendNotification = this.supportOTPProperties.getSendNotificationChannel();
        String notificationChannel = this.supportOTPProperties.getNotifyChannel();
        ...
        return supportOTPConnector.post(url, generateOtpRequest, String.class, MediaType.APPLICATION_JSON, null
                ...
                }).doAfterTerminate(() -> log.info(OTPHelper.FINALIZE)));
    }
