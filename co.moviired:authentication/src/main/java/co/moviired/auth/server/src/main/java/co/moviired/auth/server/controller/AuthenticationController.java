package co.moviired.auth.server.controller;
...
import co.moviired.auth.server.domain.repository.AuthRegisterLoginRepository;
import co.moviired.auth.server.exception.MaxDevicesException;
...
import co.moviired.auth.server.service.ServiceFactory;
...
public final class AuthenticationController {
    ...
    public  Mono<Response> login(@NotNull @RequestBody Request request) throws ParseException, JsonProcessingException {
    ...
    private Mono<Response> successAddValidations(Response response, Request request) {
        if ((request.getChannel().equals("MOVIIRED")) &&
            ...
            if ((!response.getUser().getUserType().equals(request.getSource())) && (
                    request.getSource().equals(ClientType.CHANNEL.name()) &&
                            (response.getUser().getUserType().equals("SUBSCRIBER"))
            )) {
                log.error("Intento de logeo de un usuario de origen no permitido");
                Response resp = new Response();
                resp.setErrorType(ERROR_TYPE);
                resp.setErrorCode("01");
                resp.setErrorMessage("Cuenta no existe o clave inválida.");
                return Mono.just(response);
            }
