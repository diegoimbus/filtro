...
import co.moviired.digitalcontent.business.conf.FactoriesConfig;
import co.moviired.digitalcontent.business.conf.RepositoriesConfig;
...
import static co.moviired.digitalcontent.business.domain.enums.OperationType.DIGITAL_CONTENT_PINES_RESEND;

@Slf4j
@Service
public final class DigitalContentService implements Serializable {
    ...
    private static final String HIDDEN_CLV = "password";
    private static final String HIDDEN_PIN = "pin";
    private static final String HIDDEN_MPIN = "mpin";
    private static final String HIDDEN_OTP = "otp";
    private static final String SERVICE_RESPONSE = "Respuesta del servicio:";
    ...
    public DigitalContentService(
            @NotNull FactoriesConfig fc,
            @NotNull StatusCodeConfig codeConfig,
            @NotNull PinHelper ppinHelper,
            @NotNull MahindraProperties pmahindraProperties,
            @NotNull IUserRepository iUserRepository,
            @NotNull RepositoriesConfig rc,
            @NotNull GlobalProperties globalPropertiesI) {
        super();
        this.parserFactory = fc.getParserFactory();
        this.clientFactory = fc.getClientFactory();
        this.validatorFactory = fc.getValidatorFactory();
        this.statusCodeConfig = codeConfig;
        this.pinHelper = ppinHelper;
        this.mahindraProperties = pmahindraProperties;
        this.globalProperties = globalPropertiesI;

        this.userRepository = iUserRepository;
        this.categoryRepository = rc.getCategoryRepository();
        this.productRepository = rc.getProductRepository();
        this.incommRepository = rc.getIncommRepository();
        this.conciliacionRepository = rc.getConciliacionRepository();
        this.homologationIncommRepository = rc.getHomologationIncommRepository();
        this.typeOperatorRepository = rc.getTypeOperatorRepository();
        this.subtypeOperatorRepository = rc.getSubtypeOperatorRepository();
    ...
    public Mono<DigitalContentResponse> service(@NotNull ServerHttpRequest httpRequest,
        ...
        return brequest.flatMap(request -> {
            ...
            if ((opType.equals(OperationType.DIGITAL_CONTENT_PINES_INACTIVATE)) || (opType.equals(OperationType.DIGITAL_CONTENT_CARD_INACTIVATE))) {
                Conciliacion conciliacion = generateConciliation(request);
            ...
            try {
                ...
                extraProccess(resp, opType, conciliacion, data);

                log.info(LOG_FORMATTED_2, SERVICE_RESPONSE, Security.printIgnore(this.objectMapper.writer().writeValueAsString(resp), HIDDEN_PIN, HIDDEN_MPIN, HIDDEN_OTP, HIDDEN_CLV));
    ...
    private void extraProccess(@NotNull DigitalContentResponse resp, @NotNull OperationType opType, Conciliacion conciliacion, @NotNull DigitalContentRequest data) throws DataException {
        ...
        switch (opType) {
            case DIGITAL_CONTENT_PINES_SALE:
                if (resp.getErrorCode().equalsIgnoreCase("00")) {
                    executeResponseProviderPines(data, resp, taskThread);
                }
                break;
                ...
                if (resp.getErrorCode().equalsIgnoreCase("00")) {
                    taskThread.submit(() -> {
                        if (conciliacion != null) {
                            ...
                            conciliacion.setStatusIc(statusCode.getCode());
                            Conciliacion conc = this.conciliacionRepository.save(conciliacion);
                            processReverseTransactionMahindra(data, conc);
                        }
                    });
                }
                break;
    ...
    private Conciliacion generateConciliation(@NotNull DigitalContentRequest request) {
        ...
        try {
            ...
            conciliacion.setIdTransacion(String.valueOf(new Timestamp(System.currentTimeMillis()).getTime()).substring(1));
    ...
    private Mono<DigitalContentRequest> validateParamInput(@NotNull OperationType opType,
        ...
        try {
            ...
            String requestIn = printHeader(userpass, merchantId, posId) + SERVICE_REQUEST + Security.printIgnore(this.objectMapper.writer().writeValueAsString(brequest), HIDDEN_PIN, HIDDEN_MPIN, HIDDEN_OTP, HIDDEN_CLV);
            log.info(LBL_VALUE, requestIn);

            // Validar datos obligatorios
            DigitalContentValidator validator = validatorFactory.getValidator(opType);
            validator.validationInput(brequest, merchantId, posId, userpass, true);
            ...
            // Validate phone number and email based in configuration
            brequest.setIncommConfig(getUserConfiguration(brequest));
            validator.validationInputWithConfig(brequest, brequest.getIncommConfig());

            return Mono.just(brequest);

        } catch (ParsingException | DataException | JsonProcessingException e) {
            return Mono.error(new DataException(e.getMessage(), e));
        }
    }

    /* 2 */
    private Mono<CommandResponse> authMahindra(DigitalContentRequest request) {
        try {
            // D. Realizar autenticación contra Mahindra
    ...
    /* 2 */
    private Mono<CommandResponse> validateAuthMahindra(String responseMahindraAuntentication) {
        CommandResponse mhResponse;
        try {
            // E. Transformar XML response a Mahindra response
            String mhr = responseMahindraAuntentication.replace("\n", "").replace("\r", "");

            mhResponse = this.xmlMapper.readValue(responseMahindraAuntentication, CommandResponse.class);
    ...
    private Mono<DigitalContentResponse> validateExecuteProvider(DigitalContentRequest data, String obResp, OperationType opType) {
        ...
        try {
            ...
            log.info(LOG_FORMATTED_2, "Response 'UONLNBP' Mahindra:",
                    CommandHelper.printIgnore(obResp.replace("\n", "").replace("\r", "").toUpperCase(), "TRANSACTIONCODE"));
    ...
    private void addValuesOfIva(DigitalContentRequest data, DigitalContentResponse resp) {
        try {
            if (resp.getAmount() != null) {
                if (data.getProduct().isCalculateIva()) {
                    resp.setIva(String.valueOf(Math.round(globalProperties.getIvaPercent())));
                    double totalValue = Double.parseDouble(resp.getAmount());
                    double valueBeforeIva = totalValue / ((globalProperties.getIvaPercent() / PERCENT_100) + 1D);
                    resp.setValueBeforeIva(String.valueOf(Math.round(valueBeforeIva)));
                    resp.setIvaValue(String.valueOf(Math.round(totalValue - valueBeforeIva)));
                } else {
                    resp.setIva("0");
                    resp.setIvaValue("0");
                    resp.setValueBeforeIva(resp.getAmount());
                }
            }
        } catch (Exception e) {
            log.error("Error calculating values of iva: {}", e.getMessage());
        }
    }

    private IncommConfig getUserConfiguration(DigitalContentRequest data) {
        IncommConfig incommConfig = null;
        try {
            // Obtener configuración del cliente
            Optional<User> user = userRepository.findFirstByUsernameAndPassword(
                    AESCrypt.crypt(data.getMsisdn1()), AESCrypt.crypt(data.getMpin()));
    ...
    /* 4 */
    private void executeResponseProviderPines(DigitalContentRequest data, DigitalContentResponse resp, ExecutorService taskThread) {
        try {
            // Se guarda el pin en el historial
            this.pinHelper.guardarHistorialPin(resp, data, data.getIncommConfig());

            // Se desencripta el pin encode sistema
            resp.setPin(this.pinHelper.desencriptarPinUsuario(resp.getPin()));

            String pin = String.valueOf(resp.getPin());

            // Notificar
            taskThread.submit(() -> this.pinHelper.notificarPin(resp, data, data.getIncommConfig(), pin));

            // Encriptar pin encode usuaria
            if (data.getIncommConfig() != null && data.getIncommConfig().isSendPin() && data.getIncommConfig().isEncryptPin()) {
                this.pinHelper.encriptarPinUsuario(resp, data.getIncommConfig());
            }

            if (data.getIncommCode() != null && data.getIncommConfig().isSendPin()) {
    ...
    public Mono<DigitalContentResponse> getCategories(SellerChannel source, SubtypeOperator subType) {
        ...
        try {
            ...
            if (categories.isEmpty()) {
                ...
                log.info("<== RESPONSE - GET CATEGORIES ALL [" + Security.printIgnore(this.objectMapper.writer().writeValueAsString(response), HIDDEN_PIN, HIDDEN_MPIN, HIDDEN_OTP, HIDDEN_CLV) + "]");
            ...
            for (Category category : categories) {
                ...
                category.setProducts(filterProducts(source, products));
                category.toPublic();
            }
    ...
    public DigitalContentResponse getCategory(SellerChannel source, @NotNull Integer id, SubtypeOperator subType) {
        ...
        try {
            ...
            log.info("==> RESPONSE - GET CATEGORY [" + Security.printIgnore(this.objectMapper.writer().writeValueAsString(response), HIDDEN_PIN, HIDDEN_MPIN, HIDDEN_OTP, HIDDEN_CLV) + "]");
    ...
    private List<Product> filterProducts(@NotNull SellerChannel source, List<Product> products) {
    ...
    public Mono<DigitalContentResponse> resendPin(ServerHttpRequest httpRequest, Mono<DigitalContentRequest> req, String userpass, String merchantId, String posId) {
        AtomicReference<DigitalContentRequest> requestFormatDigitalContent = new AtomicReference<>();
        return req
                .flatMap(request -> validateResendPinRequestAndDoLogin(httpRequest, request, userpass, merchantId, posId, requestFormatDigitalContent))
                .flatMap(loginResponse -> {
                    // Find pin
                    DigitalContentRequest request = requestFormatDigitalContent.get();
                    PinHistory pinHistory = this.pinHelper.findPin(request.getAuthorizationCode(), request.getTransferId());
                    if (pinHistory == null) {
                        String errorCode = String.valueOf(HttpStatus.NOT_FOUND.value());
                        return Mono.error(new ServiceException(ErrorType.VALIDATOR, errorCode, statusCodeConfig.of(errorCode).getMessage()));
                    }

                    // Notify pin
                    Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors()).submit(() -> this.pinHelper.notifyPin(request.getCorrelationId(), pinHistory));

                    // Map response
                    StatusCode statusCode = statusCodeConfig.of(String.valueOf(HttpStatus.OK.value()));
                    DigitalContentResponse resp = DigitalContentResponse.builder()
                            .errorCode(statusCode.getCode())
                            .errorMessage(statusCode.getMessage())
                            .build();
    ...
    private Mono<CommandResponse> validateResendPinRequestAndDoLogin(ServerHttpRequest httpRequest, DigitalContentRequest request, String userpass, String merchantId,
                                                                     String posId, AtomicReference<DigitalContentRequest> requestFormatDigitalContent) {
        try {
            ...
            log.info(LBL_REQUEST_START, DIGITAL_CONTENT_PINES_RESEND.name());
            String requestIn = printHeader(userpass, merchantId, posId) + SERVICE_REQUEST +
                    Security.printIgnore(this.objectMapper.writer().writeValueAsString(request), HIDDEN_PIN, HIDDEN_MPIN, HIDDEN_OTP, HIDDEN_CLV);
            log.info(LBL_VALUE, requestIn);

            DigitalContentValidator validator = validatorFactory.getValidator(DIGITAL_CONTENT_PINES_RESEND);
            validator.validationInput(request, merchantId, posId, userpass, false);

            // Validate request
            if ((request.getAuthorizationCode() == null && request.getTransferId() == null) ||
                    (request.getAuthorizationCode().trim().isEmpty() && request.getTransferId().trim().isEmpty())) {
                String errorCode = String.valueOf(HttpStatus.BAD_REQUEST.value());
                return Mono.error(new ServiceException(ErrorType.VALIDATOR, errorCode, statusCodeConfig.of(errorCode).getMessage()));
            }

            if (request.getAuthorizationCode() == null) {
                request.setAuthorizationCode("-1");
            }

            if (request.getTransferId() == null) {
                request.setTransferId("-1");
            }
    ...
    private Mono<DigitalContentResponse> onError(Throwable e) {
        StatusCode statusCode = new StatusCode();
        statusCode.setLevel(StatusCode.Level.FAIL);

        if (e instanceof ServiceException) {
            ServiceException se = (ServiceException) e;
            statusCode = statusCodeConfig.of(se.getCode(), se.getMessage());
        } else {
        ...
        try {
            log.error(LOG_FORMATTED_2, SERVICE_RESPONSE, Security.printIgnore(this.objectMapper.writer().writeValueAsString(resp), HIDDEN_PIN, HIDDEN_MPIN, HIDDEN_OTP, HIDDEN_CLV));
        } catch (JsonProcessingException ex) {
            return Mono.just(resp);
        }
        return Mono.just(resp);
